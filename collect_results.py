...
def combine_particle_results_arbitrary(trajectories, fit_results_alpha_D_app_individual, spot_type, label = None, avoid = None, transfer_columns = ['starting_snr', 'average_starting_magnitude', 'average_starting_offset', 'below_diffraction_limit']):
    '''
    Same as below but we don't think that the particle ID is in the index. Here it can be anywhere
    in the dataframe.
    ...
    filename = 'combined_results_per_particle_arbitrary' + label  +'.pkl'
    ...
    for j, k in list(enumerate(keys_of_interest))[:]:  # note that the keys of the two dictionaries may not always be the same
        ...
        for p in set(fit_results_alpha_D_app_individual[k].particle):
            for q in transfer_columns:
                results[k].loc[results[k].particle == p, q] = trajectories[k].loc[trajectories[k].particle == p, q].to_numpy().mean()#[0]  # only transfering the first entry for this particle's quantity, since we are only interested in quantities that have the same value across all frames
...
def combine_particle_results_inverse(trajectories_new, fit_results_new, spot_type, label = None, avoid = None, transfer_columns = ['starting_snr', 'average_starting_magnitude', 'average_starting_offset', 'below_diffraction_limit']):
    '''
    This one pulls fit results into the trajectories, not as above where trajectory properties are pulled into the fit_results.
    Combine per-particle information included in the trajectories DataFrames with information in the DataFrames you obtained when you fit to the msds (for instance using fit_imsds_within_time_range() from process_msds.py).
    This pertains to information that characterizes an entire particle, i.e. not information that characterizes a particle in a certain frame.
    ...
    filename = 'combined_results_per_trajectory' + label + '.pkl'

    keys_of_interest = list(fit_results_new.keys())
    ...
    for i in keys_of_interest[:]:
        if i not in list(trajectories_new.keys()):
            print(i + ' is included in the trajectories but not in the fit results for α, D_app.')

    for i in list(trajectories_new.keys()):
        if i not in keys_of_interest:
            print(i + ' is included in the fit results for α, D_app but not in the trajectories.')


    keys_of_interest = [x for x in keys_of_interest if x in list(trajectories_new.keys())]
    ...
    for j, k in list(enumerate(keys_of_interest))[:]:  # note that the keys of the two dictionaries may not always be the same
        ...
        d = load_all_results.read('day', k, spot_type)
        s = load_all_results.read('starvation_time', k, spot_type) + 'h'
        for p in tqdm(trajectories_new[k].particle):
            for q in transfer_columns:
                parameter = fit_results_new[k].loc[fit_results_new[k].index == p, q].to_numpy().mean()
                results[k].loc[results[k].particle == p, q] = parameter#[0]  # only transfering the first entry for this particle's quantity, since we are only interested in quantities that have the same value across all frames
...
def allocate_movies_to_categories(spot_type, movies = 'all_movies', time_between_frames = ['5000'], strains = ['bLR1', 'bLR2'], starvation_times = all_starvation_times, conditions = ['lowN', 'lowC']):
    ...
    avoid = data[spot_type]['typically_avoid']
    ...
    # Save to archive with date time
    os.makedirs(basic_directory + '/_' + spot_type + '/' + 'archive/', exist_ok=True)
    numpy.save(basic_directory + '/_' + spot_type + '/' + 'archive/' + 'pooled_directories_by_day_' + string_now + '.npy', pooled_directories_by_day)
    numpy.save(basic_directory + '/_' + spot_type + '/' + 'archive/' + 'pooled_directories_by_day_without_timelag_' + string_now + '.npy', pooled_directories_by_day_without_timelag)
    numpy.save(basic_directory + '/_' + spot_type + '/' + 'archive/' + 'pooled_directories_' + string_now + '.npy', pooled_directories)
    numpy.save(basic_directory + '/_' + spot_type + '/' + 'archive/' + 'pooled_directories_without_timelag_' + string_now + '.npy', pooled_directories_without_timelag)
    #Save to main folder to be used immediately
    numpy.save(basic_directory + '/_' + spot_type + '/' + 'pooled_directories_by_day' + '.npy', pooled_directories_by_day)
    numpy.save(basic_directory + '/_' + spot_type + '/' + 'pooled_directories_by_day_without_timelag' + '.npy', pooled_directories_by_day_without_timelag)
    numpy.save(basic_directory + '/_' + spot_type + '/' + 'pooled_directories' + '.npy', pooled_directories)
    numpy.save(basic_directory + '/_' + spot_type + '/' + 'pooled_directories_without_timelag' + '.npy', pooled_directories_without_timelag)
